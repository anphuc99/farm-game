...
public class ReadGoogleSheets
{
    ...
    public static Task<List<List<string>>> GetTable(string id, string gridId)
    {
        TaskCompletionSource<List<List<string>>> tcp = new TaskCompletionSource<List<List<string>>>();
        GetTable(id, gridId, (items) => tcp.SetResult(items));
        return tcp.Task;
    }
